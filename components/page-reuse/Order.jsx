...
const CreateOrder = ({ flag, setFlag, refetch }) => {
  ...
  useEffect(() => {
    const searcher = new FuzzySearch(items, ['name', 'price', 'source', 'description', 'category.name'], {caseSensitive: false})
    const result = searcher.search(search)
    setSearchedItems(result)
  }, [search])
  useEffect(() => {
    const data = products.filter(product => !product.selected)
    setItems([...data])
                ...
                selectedItems.length > 0 ? (
                  ...
                  >
                      ...
                      products.filter(product => product.selected).map(
                        product => (
                          ...
                          >
                            ...
                            <div className="w-20 flex-none">{formatCurrency(product.price)}</div>
                            ...
                            <div className="grow">{formatCurrency(product.price)}</div>
                            <button
                    ...
                    <li className="min-w-full py-2 px-5">
                      <h2 className="text-center font-medium text-red-500">Amount: {formatCurrency(totalAmount)}</h2>
                    </li>
                  </ul>
                ) : null
              }
              ...
              items.length > 0 ? (
                <div className="space-y-2">
                  <button
                    ...
                    onClick={() => setIsAddItemShow(true)}
                  >
                    Add Items
                  </button>
                  <div className={`w-full ${isAddItemShow === true ? '' : 'hidden'}`}>
                    <TextInput
                      id="search"
                      name="search"
                    ...
                    />
                    <ul
                      className="min-w-full w-max overflow-auto text-gray-800 transition-all duration-300 border rounded shadow"
                    >
                      <li className="min-w-full flex space-x-5 w-max border-b py-2 px-5">
                        <div className="w-56 font-medium flex-none">Name</div>
  ...
  return (
    <li className="min-w-full">
      ...
      >
        ...
        <div className="w-40 flex-none">{formatCurrency(order.amount)}</div>
          ...
          bodyTemplate={(
            ...
            >
              ...
              <div className="flex">
                ...
                <p className="grow"> {formatCurrency(order.amount)}</p>
              </div>
              <div className="flex">
                  ...
                  order.orderDetails.map(
                    orderDetail => (
                      <div key={orderDetail.id} className="border-b px-2 py-1.5 flex space-x-2">
                        ...
                        <p className="w-32 flex-none">{formatCurrency(orderDetail.product.price)}</p>
                        <p className="w-32 flex-none">{orderDetail.quantity}</p>
                        <p className="grow">{formatCurrency(orderDetail.price)}</p>
                      </div>
                    )
                  )
                }
